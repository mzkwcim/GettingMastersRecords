...
namespace GettingMastersRecords
{
    internal class ListGettingSystem
    {
        ...
        {
            return Convert.ToInt32(ScrapingSystem.Loader(s).DocumentNode.SelectNodes("//td[@class='rankingPlace']")[(gaga * 5) - 4].InnerText);
        }
        public static List<string> BirthDates(string s)
        {
            double places = ScrapingSystem.GetPlacesModulo25Celling(s);
        ...
        public static HtmlAgilityPack.HtmlNodeCollection HelperList(int counter, string s, string helper) => ScrapingSystem.Loader((counter == 1) ? s.Replace("firstPlace=1", $"firstPlace={counter}") : s.Replace($"firstPlace={counter - 25}", $"firstPlace={counter}")).DocumentNode.SelectNodes(helper);
